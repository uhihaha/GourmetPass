...
<style>
    ...
    .signup-table th { width: 25%; padding: 15px 10px; text-align: left; vertical-align: middle; font-size: 14px; color: #333; border-bottom: 1px solid #eee; }
    .signup-table td { width: 75%; padding: 15px 10px; vertical-align: middle; border-bottom: 1px solid #eee; }
    
    .signup-input { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 15px; box-sizing: border-box; }
    .signup-input:focus { border-color: #333; outline: none; }
    
    .input-row { display: flex; gap: 10px; align-items: center; }
    .btn-wire { padding: 12px 15px; border: 1px solid #333; border-radius: 8px; background: #fff; font-weight: bold; cursor: pointer; white-space: nowrap; }
    
    .btn-group { display: flex; gap: 15px; margin-top: 40px; justify-content: center; }
    .btn-submit { width: 180px; padding: 15px; background: #333; color: #fff; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; }
    .btn-cancel { width: 180px; padding: 15px; background: #eee; color: #333; border: none; border-radius: 8px; font-weight: bold; text-align: center; text-decoration: none; }
    .msg-box { font-size: 12px; margin-top: 5px; display: block; }
</style>

<div class="signup-wrapper">
    <div class="signup-title">👤 일반 회원가입</div>
    ...
    <form action="${pageContext.request.contextPath}/member/joinProcess" method="post" id="joinForm">
        ...
        <table class="signup-table">
            ...
            <tr>
                <th>전화번호</th>
                <td><input type="text" name="user_tel" class="signup-input" required placeholder="숫자만 입력" maxlength="13" oninput="autoHyphen(this)"></td>
            </tr>
            <tr>
                <th>주소</th>
                <td>
                    <div class="input-row">
                        <input type="text" name="user_zip" id="user_zip" class="signup-input" style="width: 120px; flex: none;" placeholder="우편번호" readonly>
                        <button type="button" onclick="execDaumPostcode('user')" class="btn-wire">주소검색</button>
                    </div>
                    <input type="text" name="user_addr1" id="user_addr1" class="signup-input" style="margin-top:8px;" placeholder="기본주소" readonly>
