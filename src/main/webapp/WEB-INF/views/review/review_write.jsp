<%-- 
    GourmetPass 프로젝트: 리뷰 작성 페이지
    - REVIEW 테이블 데이터 생성을 위한 입력 폼입니다.
    - Spring Message Tag (text 속성) 적용으로 다국어 지원 및 안정성을 확보했습니다.
--%>
...
<div style="width: 80%; max-width: 600px; margin: 40px auto; padding: 30px; border: 2px solid #333; border-radius: 15px; background: #fff;">
    <h2 style="text-align: center; margin-bottom: 30px;">
        <spring:message code="review.write.title" text="✍️ 리뷰 작성하기" />
    </h2>
    
    <div style="background: #f9f9f9; padding: 15px; border-radius: 8px; margin-bottom: 25px; border: 1px solid #ddd;">
        <p style="margin: 0; color: #666; font-size: 14px;">
            <spring:message code="review.write.store_label" text="방문하신 매장" />
        </p>
    ...
    <form action="${pageContext.request.contextPath}/review/write" method="post">
        ...
        <div style="margin-bottom: 25px;">
            <label style="display: block; font-weight: bold; margin-bottom: 10px;">
                <spring:message code="review.write.rating_label" text="⭐ 별점을 선택해 주세요" />
            </label>
        ...
        <div style="margin-bottom: 25px;">
            <label style="display: block; font-weight: bold; margin-bottom: 10px;">
                <spring:message code="review.write.content_label" text="💬 방문 후기" />
            </label>
            <textarea name="content" required 
                      placeholder="<spring:message code='review.write.placeholder' text='맛, 서비스, 분위기는 어떠셨나요? 솔직한 후기를 남겨주세요.' />" 
        ...
        <div style="display: flex; gap: 15px; margin-top: 30px;">
            ...
            <button type="submit" 
                ...
                <spring:message code="review.btn.submit" text="리뷰 등록하기" />
            </button>
        </div>
    </form>
</div>
