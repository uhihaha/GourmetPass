<%-- WEB-INF/views/main.jsp --%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn"%>

<jsp:include page="common/header.jsp" />

<%-- [원칙 1] 표준 스타일시트 및 메인 전용 스타일 연결 --%>
<link rel="stylesheet" href="<c:url value='/resources/css/member.css'/>">
<link rel="stylesheet" href="<c:url value='/resources/css/main.css'/>">

<div class="main-wrapper">
    <%-- 1. 검색 섹션: Bold Wire 표준 카드 적용 --%>
    <div class="search-card">
        <h1 class="search-title">🍴 오늘 어떤 맛집을 예약할까요?</h1>
        <form action="${pageContext.request.contextPath}/store/list" method="get" class="search-form">
            <input type="text" name="keyword" class="search-input" placeholder="가게 이름 또는 메뉴 검색">
            <button type="submit" class="btn-search">맛집 검색</button>
        </form>
    </div>

    <%-- 2. 카테고리 섹션: store_list와 동일한 칩(Chip) 디자인 적용 --%>
    <div class="category-section">
        <div class="chip-group">
            <c:set var="categories" value="한식,일식,양식,중식,카페" />
            <c:forEach var="cat" items="${fn:split(categories, ',')}">
                <div class="cat-chip clickable" data-url="/store/list?category=${cat}">${cat}</div>
            </c:forEach>
            <div class="cat-chip btn-all clickable" data-url="/store/list">전체보기</div>
        </div>
    </div>

    <%-- 섹션 구분선: 인라인 스타일 제거 --%>
    <div class="main-divider"></div>

    <%-- 3. 실시간 인기 맛집 섹션 --%>
    <div class="popular-section">
        <h2 class="section-title">🔥 실시간 인기 맛집 (TOP 6)</h2>

        <div class="store-grid">
            <c:choose>
                <c:when test="${not empty storeList}">
                    <c:forEach var="store" items="${storeList}" end="5">
                        <%-- 맛집 카드: clickable 클래스와 data-url로 관심사 분리 --%>
                        <div class="store-card clickable" data-url="/store/detail?storeId=${store.store_id}">
                            <div class="store-img-box">
                                <c:choose>
                                    <c:when test="${not empty store.store_img}">
                                        <img src="${pageContext.request.contextPath}/upload/${store.store_img}" alt="${store.store_name}">
                                    </c:when>
                                    <c:otherwise>
                                        <div class="no-img-text">NO IMAGE</div>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                            <div class="store-info">
                                <div class="store-cat">${store.store_category}</div>
                                <div class="store-name-row">
                                    <h3 class="store-name">${store.store_name}</h3>
                                    <c:if test="${store.store_cnt >= 100}">
                                        <span class="hot-badge">HOT</span>
                                    </c:if>
                                </div>
                                <div class="store-addr-text">${store.store_addr1}</div>
                                <%-- 통계 정보: 별점 및 조회수 규격화 --%>
                                <div class="store-stats">
                                    <span class="stat-rating">⭐ ${store.avg_rating} <small>(${store.review_cnt})</small></span>
                                    <span class="stat-views">👀 조회수 ${store.store_cnt}</span>
                                </div>
                            </div>
                        </div>
                    </c:forEach>
                </c:when>
                <c:otherwise>
                    <%-- 데이터 없을 때 표시되는 표준 가이드 영역 --%>
                    <div class="empty-status-box" style="grid-column: span 3;">
                        현재 등록된 인기 맛집 정보가 없습니다.
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
    </div>
</div>

<%-- [원칙 1] 공통 인터랙션 스크립트 연결 --%>
<script src="<c:url value='/resources/js/main.js'/>"></script>
<jsp:include page="common/footer.jsp" />