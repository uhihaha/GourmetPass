...
<script>
    const APP_CONFIG = {
        ...
        activeStoreId: "${not empty activeWait ? activeWait.store_id : (not empty activeBook ? activeBook.store_id : '')}"
    };
...
<div class="edit-wrapper" style="max-width: 900px;">
    ...
    <div class="dashboard-card">
        ...
        <c:choose>
                    ...
                    <div class="item-card status-card ${activeWait.wait_status == 'ING' ? 'dining-mode' : ''}">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="history-info">
                                <c:choose>
                                    <c:when test="${activeWait.wait_status == 'ING'}"><span class="badge-wire badge-ing">🍽️ 식사 중</span></c:when>
                                    <c:when test="${activeWait.wait_status == 'CALLED'}"><span class="badge-wire badge-call">📢 입장 호출!</span></c:when>
                                    <c:otherwise><span class="badge-wire">🚶 웨이팅 중</span></c:otherwise>
                                </c:choose>
                                <h3 style="font-size: 22px; font-weight: 900; margin: 10px 0;">${activeWait.store_name}</h3>
                                <p style="font-size: 15px; color: #555;">
                                    <c:choose>
                                        <c:when test="${activeWait.wait_status == 'ING'}"><span class="dining-msg">맛있는 식사 되세요!</span></c:when>
                            ...
                            <div class="history-actions">
                                <c:if test="${activeWait.wait_status == 'WAITING'}">
                                    <button type="button" class="btn-wire" style="color: #dc3545; border-color: #dc3545;" onclick="cancelWait('${activeWait.wait_id}')">웨이팅 취소</button>
                                </c:if>
                                <c:if test="${activeWait.wait_status == 'ING'}">
                                    <button class="btn-small btn-payment" onclick="alert('결제 상세 기능 준비 중입니다.')">주문 확인</button>
                                </c:if>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="item-card status-card">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="history-info">
                                ...
                                <p style="font-size: 15px; color: #555;">
                                    방문 일시: <b><fmt:formatDate value="${activeBook.book_date}" pattern="MM월 dd일 HH:mm"/></b>
                                </p>
                            </div>
                            <div class="history-actions">
                                <span style="font-weight: 900; font-size: 14px; color: #2e7d32;">방문 예정</span>
                            </div>
                        </div>
                    </div>
                </c:if>
            ...
            <c:otherwise>
                <div style="text-align: center; padding: 50px 0; color: #999; font-weight: 800;">현재 이용 중인 서비스가 없습니다.</div>
            </c:otherwise>
        </c:choose>
    </div>
                ...
                <div class="history-item">
                    ...
                    <div class="history-actions">
                        ...
                        <c:if test="${w.wait_status == 'FINISH'}">
                            <button class="btn-small btn-payment" onclick="alert('결제/영수증 상세 페이지로 이동합니다.')">결제내역</button>
                            <c:choose>
                                <c:when test="${empty w.review_id}">
                                    <button class="btn-small btn-review" onclick="location.href='<c:url value='/review/write?store_id=${w.store_id}&wait_id=${w.wait_id}'/>'">리뷰 작성</button>
                                </c:when>
                                <c:otherwise><span class="text-done">리뷰완료</span></c:otherwise>
                            </c:choose>
                        </c:if>
                        <c:if test="${w.wait_status == 'CANCELLED'}"><span class="text-done" style="color:#dc3545">취소됨</span></c:if>
                    </div>
                </div>
            </c:forEach>
                ...
                <div class="history-item">
                    ...
                    <div class="history-actions">
                        <c:if test="${b.book_status == 'FINISH'}">
                            <button class="btn-small btn-payment" onclick="alert('결제 상세 정보를 확인합니다.')">결제내역</button>
                            <c:choose>
                                <c:when test="${empty b.review_id}">
                                    <button class="btn-small btn-review" onclick="location.href='<c:url value='/review/write?store_id=${b.store_id}&book_id=${b.book_id}'/>'">리뷰 작성</button>
                                </c:when>
                                <c:otherwise><span class="text-done">리뷰완료</span></c:otherwise>
                            </c:choose>
                        </c:if>
                        <c:if test="${b.book_status == 'RESERVED'}"><span class="text-done" style="color:#2e7d32">방문예정</span></c:if>
                    </div>
                </div>
            </c:forEach>
        </div>
